...
def get_grid_masks(gridO, gridI):
    ...
    masks.append(
        sparse.RightRepetitiveMask.get_grid_mask_from_1d(gridI, nO=gridO))
    ...
    masks.append(
        sparse.LeftRepetitiveMask.get_grid_mask_from_1d(gridI, nO=gridO))
...
class Self_Attn(nn.Module):
    ...
    def __init__(self, in_channels):
        ...
        self.snconv1x1_g = snconv2d(
            in_channels=in_channels, out_channels=in_channels//2, kernel_size=1, stride=1, padding=0)
        self.snconv1x1_attn = snconv2d(
            in_channels=in_channels//2, out_channels=in_channels, kernel_size=1, stride=1, padding=0)
...
class Self_Attn_ylg(nn.Module):
    ...
    def forward(self, x):
        ...
        # Theta path
        theta = self.snconv1x1_theta(x)
        theta = theta.view(-1, 8, ch//(8*8), h*w)
        # Phi path
        phi = self.snconv1x1_phi(x)
        phi = self.maxpool(phi)
...
class ConditionalBatchNorm2d(nn.Module):
    ...
    def __init__(self, num_features, num_classes):
        ...
        self.embed.weight.data[:, :num_features].fill_(
            1.)  # Initialize scale to 1
...
class Generator(nn.Module):
    ...
    def __init__(self, z_dim, g_conv_dim, num_classes):
        ...
        self.snlinear0 = snlinear(
            in_features=z_dim, out_features=g_conv_dim*16*4*4)  # 64*16=1024
        self.block1 = GenBlock(g_conv_dim*16, g_conv_dim *
                               16, num_classes)  # 1024 8*8
        ...
        self.bn = nn.BatchNorm2d(
            g_conv_dim*4, eps=1e-5, momentum=0.0001, affine=True)
        self.relu = nn.ReLU(inplace=True)
        self.snconv2d1 = snconv2d(
            in_channels=g_conv_dim*4, out_channels=3, kernel_size=3, stride=1, padding=1)
        self.tanh = nn.Tanh()
    ...
    def forward(self, z, labels):
        ...
        act3 = self.block4(act2, labels)    # n x g_conv_dim*2 x 64 x 64
        # act5 = self.block5(act4, labels)    # n x g_conv_dim  x 128 x 128
        act4, attn = self.self_attn(act3)         # n x g_conv_dim*4 x 32 x 32

        act5 = self.bn(act4)                # n x g_conv_dim  x 128 x 128
...
class Discriminator(nn.Module):
    ...
    def __init__(self, d_conv_dim, num_classes):
        ...
        self.block5 = DiscBlock(d_conv_dim*4, d_conv_dim*8)
        self.relu = nn.ReLU(inplace=True)
    ...
    def forward(self, x, labels):
        ...
        # h1, attn = self.self_attn(h1)  # n x d_conv_dim*2 x 32 x 32
        ...
        h5 = self.block5(h2, downsample=False)  # n x d_conv_dim*16 x 4 x 4
