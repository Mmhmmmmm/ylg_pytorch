...
def get_parameters():
    ...
    parser.add_argument('--dataset', type=str, default='cifar10', choices=["cifar10", "fake", "folder", "hdf5", "imagenet", "lfw", "lsun"],
    ...
    parser.add_argument('--total_step', type=int,
                        default=200000, help='how many iterations')
    parser.add_argument('--d_steps_per_iter', type=int,
                        default=1, help='how many D updates per iteration')
    parser.add_argument('--g_steps_per_iter', type=int,
                        default=1, help='how many G updates per iteration')
    ...
    parser.add_argument('--adv_loss', type=str, default='hinge',
                        choices=['hinge', 'dcgan', 'wgan_gp', 'gan'])
    ...
    parser.add_argument('--dont_drop_last', action='store_true',
                        help="Whether not to drop the last batch in dataset if its size < batch_size")
    parser.add_argument('--dont_resize', action='store_true',
                        help="Whether not to resize images")
    parser.add_argument('--imsize', type=int, default=32)
    parser.add_argument('--centercrop', action='store_true',
                        help="Whether to center crop images")
    parser.add_argument('--centercrop_size', type=int, default=32)
    parser.add_argument('--dont_normalize', action='store_true',
                        help="Whether to normalize image values")

    # Step sizes
    parser.add_argument('--log_step', type=int, default=100)
    parser.add_argument('--sample_step', type=int, default=1000)
    parser.add_argument('--model_save_step', type=float, default=10000)
    ...
    parser.add_argument('--pretrained_model', type=str,
                        default='')
    parser.add_argument('--state_dict_or_model', type=str, default='state_dict',
                        help="Specify whether .pth pretrained_model is a 'state_dict' or a complete 'model'")
    ...
    assert args.batch_size_in_gpu <= args.batch_size, "ERROR: please make sure batch_size >= batch_size_in_gpu!! Given batch_size: " + \
        str(args.batch_size) + " ; batch_size_in_gpu: " + \
        str(args.batch_size_in_gpu)
    assert args.batch_size % args.batch_size_in_gpu == 0, "ERROR: please make sure batch_size_in_gpu divides batch_size!! Given batch_size: " + \
        str(args.batch_size) + " ; batch_size_in_gpu: " + \
        str(args.batch_size_in_gpu)

    args.batch_size_effective = args.batch_size_in_gpu * \
        (args.batch_size//args.batch_size_in_gpu)
    ...
    assert args.save_n_images <= args.batch_size_in_gpu, "ERROR: please make save_n_images <= batch_size_in_gpu!! Given save_n_images: " + \
        str(args.save_n_images) + " ; batch_size_in_gpu: " + \
        str(args.batch_size_in_gpu)
    ...
    args.name = '{0:%Y%m%d_%H%M%S}_{1}_{2}'.format(
        datetime.datetime.now(), args.name, os.path.basename(args.data_path))

    args.save_path = os.path.join(args.save_path, args.name)
    args.model_weights_path = os.path.join(
        args.save_path, args.model_weights_dir)
    args.sample_images_path = os.path.join(
        args.save_path, args.sample_images_dir)
