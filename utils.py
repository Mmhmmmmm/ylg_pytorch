...
def make_dataloader(batch_size, dataset_type, data_path, shuffle=True, drop_last=True, dataloader_args={},
    ...
    if dataset_type in ['folder', 'imagenet', 'lfw']:
        ...
        assert os.path.exists(
            data_path), "data_path does not exist! Given: " + data_path
        dataset = dset.ImageFolder(root=data_path, transform=transform)
    elif dataset_type == 'lsun':
        assert os.path.exists(
            data_path), "data_path does not exist! Given: " + data_path
        dataset = dset.LSUN(root=data_path, classes=[
                            'bedroom_train'], transform=transform)
        ...
        dataset = dset.FakeData(image_size=(
            3, centercrop_size, centercrop_size), transform=transforms.ToTensor())
    assert dataset
    num_of_classes = len(dataset.classes)
    print("Data found!  # of images =", len(dataset), ", # of classes =",
          num_of_classes, ", classes:", dataset.classes)
    # Make dataloader from dataset
    dataloader = torch.utils.data.DataLoader(
        dataset, batch_size=batch_size, shuffle=shuffle, drop_last=drop_last, **dataloader_args)
...
def make_gif(image, iteration_number, save_path, model_name, max_frames_per_gif=100):
    ...
    # Read old gif frames
    try:
        gif_frames_reader = imageio.get_reader(
            os.path.join(save_path, model_name + ".gif"))
    ...
    if len(gif_frames) > max_frames_per_gif:
        ...
        num_of_gifs_already_saved = len(
            glob.glob(os.path.join(save_path, model_name + "_*.gif")))
        print("Saving", os.path.join(save_path, model_name +
                                     "_%05d.gif" % (num_of_gifs_already_saved)))
        imageio.mimsave(os.path.join(save_path, model_name + "_%05d.gif" %
                                     (num_of_gifs_already_saved)), gif_frames_00)
...
def save_ckpt(sagan_obj, model=False, final=False):
    ...
    if final:
        ...
        torch.save({
            'step': sagan_obj.step,
            # "module" in case DataParallel is used
...
def load_pretrained_model(sagan_obj):
    ...
    # Check for path
    assert os.path.exists(
        sagan_obj.config.pretrained_model), "Path of .pth pretrained_model doesn't exist! Given: " + sagan_obj.config.pretrained_model
    ...
    if sagan_obj.config.state_dict_or_model == 'state_dict':
        ...
        sagan_obj.G_optimizer.load_state_dict(
            checkpoint['G_optimizer_state_dict'])
        sagan_obj.D.load_state_dict(checkpoint['D_state_dict'])
        sagan_obj.D_optimizer.load_state_dict(
            checkpoint['D_optimizer_state_dict'])
