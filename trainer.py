...
class Trainer(object):
    ...
    def train(self):
        ...
        fixed_noise = torch.randn(
            self.config.batch_size_in_gpu, self.config.z_dim, device=self.device)
        if self.num_of_classes < self.config.batch_size_in_gpu:
            fixed_labels = torch.from_numpy(np.tile(np.arange(
                self.num_of_classes), self.config.batch_size_in_gpu//self.num_of_classes + 1)[:self.config.batch_size_in_gpu]).to(self.device)
        else:
            fixed_labels = torch.from_numpy(
                np.arange(self.config.batch_size_in_gpu)).to(self.device)

        # For gan loss
        label = torch.full((self.config.batch_size_in_gpu,),
                           1, device=self.device, dtype=torch.float)
        ones = torch.full((self.config.batch_size_in_gpu,), 1,
        ...
        inst_noise_mean = torch.full((self.config.batch_size_in_gpu, 3, self.config.imsize,
                                      self.config.imsize), 0, device=self.device, dtype=torch.float)
        inst_noise_std = torch.full((self.config.batch_size_in_gpu, 3, self.config.imsize,
                                     self.config.imsize), self.config.inst_noise_sigma, device=self.device, dtype=torch.float)
        ...
        for self.step in range(self.start, self.config.total_step):
            ...
            for _ in range(self.config.d_steps_per_iter):
                ...
                for gpu_batch in range(self.gpu_batches):
                    ...
                    d_out_real = self.D(
                        real_images + inst_noise, real_labels)
                    ...
                    # Create random noise
                    z = torch.randn(self.config.batch_size_in_gpu,
                                    self.config.z_dim, device=self.device)

                    # Generate fake images for same real labels
                    fake_images, _ = self.G(z, real_labels)

                    # Get D output for fake images & same real labels
                    inst_noise = torch.normal(
                        mean=inst_noise_mean, std=inst_noise_std).to(self.device)
                    d_out_fake = self.D(
                        fake_images.detach() + inst_noise, real_labels)
                    ...
                    if self.config.adv_loss == 'wgan_gp':
                        d_loss_gp = self.config.lambda_gp * \
            ...
            for _ in range(self.config.g_steps_per_iter):
                ...
                for gpu_batch in range(self.gpu_batches):
                    ...
                    # Create random noise
                    z = torch.randn(self.config.batch_size_in_gpu,
                                    self.config.z_dim).to(self.device)
                    ...
                    inst_noise = torch.normal(
                        mean=inst_noise_mean, std=inst_noise_std).to(self.device)
                    g_out_fake = self.D(
                        fake_images + inst_noise, real_labels)
            ...
            if self.step % self.config.sample_step == 0:
                ...
                self.G.eval()
                fake_images, _ = self.G(fixed_noise, fixed_labels)
                self.G.train()
                sample_images = utils.denorm(
                    fake_images.detach()[:self.config.save_n_images])
                # Save batch images
                vutils.save_image(sample_images, os.path.join(
                    self.config.sample_images_path, 'fake_{:05d}.png'.format(self.step)), nrow=self.config.nrow)
                # Save gif
    ...
    def get_real_samples(self):
        ...
        real_images, real_labels = real_images.to(
    ...
    def compute_gradient_penalty(self, real_images, real_labels, fake_images):
        ...
        alpha = torch.rand(real_images.size(0), 1, 1,
                           1).expand_as(real_images).to(self.device)
        interpolated = torch.tensor(
            alpha * real_images + (1 - alpha) * fake_images, requires_grad=True)
        out, _ = self.D(interpolated, real_labels)
